...
class UpcomingView(View):
    ...
    template_name_2 = 'booking/upcoming_trains.html'

    def get(self, request):
        if is_authenticated(request) == None:
            return redirect('accounts:Login')
        else:
            if request.GET.get('type') == 'hotels':
            ...
            elif request.GET.get('type') == 'buses':
            ...
            elif request.GET.get('type') == 'trains':
                train_bookings = utils.get_train_booking_by_user(request.session.get('email'))
                upcoming_bookings = []
                for booking in train_bookings:
                    ...
                    if date < datetime.date.today():
                        ...
                        train_detail = models.ServiceMetaData.objects.get(id = booking.get('service_id'))
                        booking.update({'service_name': train_detail.name})
                        upcoming_bookings.append(booking)
                return render(request, self.template_name_2, {'train_bookings': upcoming_bookings, 'type': get_type(request)})

class PastView(View):

    template_name = 'booking/past_hotels.html'
    template_name_1 = 'booking/past_buses.html'
    template_name_2 = 'booking/past_trains.html'
...
class BookingDetailView(View):
    ...
    template_name_2 = 'booking/details_train.html'
        ...
        if is_authenticated(request) == None:
            ...
            if id[0] == 'H':
                ...
                if not booking_details:
                    ...
                    try:
                        booking = utils.get_train_booking_by_id(id)
                        TravelDate = datetime.datetime.strptime(booking.get('TravelDate'), "%Y-%m-%d").date()
                        cancel_option = datetime.date.today() < TravelDate
                        if booking.get('email') != request.session.get('email'):
                            print('NOT FOUND')
                        else:
                            train = models.ServiceMetaData.objects.get(id = booking.get('service_id'))
                            return render(request, self.template_name_1, {'form': form, 'booking': booking, 'train': train, 'cancel_option': cancel_option, 'type': get_type(request)})
                    except:
                        print('INTERNAL ERROR')
                ...
                if form.is_valid():
                    ...
                    if check_password(form.cleaned_data.get('password'), user.get('password')) == True:
                        ...
                        if r == 200:
                            ...
                            return render(request, self.template_name_2, {'form': forms.DeleteBookingForm(), 'error': '1', 'msg': 'Internal Error! Try again after some time.', 'booking': booking, 'cancel_option': cancel_option, 'train': train, 'type': get_type(request)})
                    else:
                        return render(request, self.template_name_2, {'form': forms.DeleteBookingForm(), 'error': '1', 'msg': 'Incorrect Password', 'booking': booking, 'cancel_option': cancel_option, 'train': train, 'type': get_type(request)})
                else:
                    return render(request, self.template_name_2, {'form': forms.DeleteBookingForm(), 'error': '1', 'msg': 'Fields cannot be empty', 'cancel_option': cancel_option, 'booking': booking, 'train': train, 'type': get_type(request)})
